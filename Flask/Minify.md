...
이와 같은 것을 `Minify` 코드 경량화라고 하는데, 코드 경량화를 하는 방법은 굉장히 많다. 그 중 `python`을 사용해서 `Minify`하는 코드를 아래에 작성했다. 사용된 라이브러리는 `rjsmin`과 `rcssmin`이다. (라이브러리명을 보면 알 수 있듯이 rjsmin은 js를 경량화하며, rcssmin은 css를 경량화한다.)
